				...
				<section class="vbox">
					<section class="scrollable bg-white">
						<div class="content">
							...
							<?php $this->load->view('admin/_common/search_bar', array('title' => $group_name)); ?>
										...
										<table class="table">
											...
											<?php foreach ($contacts as $row) : ?>
												<tr class="contacts" id="contact-<?php echo $row['contact_id']; ?>">
													...
													<td>
														<?php echo anchor('backend/account/edit/' . $row['contact_id'] . '?return=backend/providers', $row['first_name'] . ' ' . $row['last_name'], 'class="text-regular"'); ?>
													</td>
														...
														<?php if (isset($row['services'])) : ?>
															<?php foreach ($row['services'] as $service_id => $service) : ?>
																<div class="container-fluid"><span
																		class="label label-primary"><?php echo $service; ?>
																		<a data-toggle="modal"
																		   data-target="#modal-popup"
																		   style="color:#fff;"
																		   href="<?php echo site_url('backend/service/delete_user/' . $row['contact_id'] . '/' . $service_id); ?>"><i
													...
													<td class="text-center">
														<a href="<?php echo site_url('backend/account/edit/' . $row['contact_id']); ?>?return=backend/providers"
														   class="btn btn-xs btn-icon btn-primary"><i
																class="fa fa-pencil"></i></a>
														<a href="<?php echo site_url('backend/account/delete/' . $row['contact_id']); ?>"
														   class="btn btn-xs btn-icon btn-default btn-confirm"
														   title="Are you sure you want to delete this person?"><i
